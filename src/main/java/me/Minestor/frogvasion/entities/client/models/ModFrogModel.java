...
public class ModFrogModel<T extends ModFrog> extends SinglePartEntityModel<T> {
    private final ModelPart frog;
    private final ModelPart eyes;
    public ModFrogModel(ModelPart root) {
        this.frog = root.getChild("frog");
        this.eyes = frog.getChild("body").getChild("head").getChild("eyes");
    }
    public static TexturedModelData getTexturedModelData() {
        ...
        ModelPartData eyes = head.addChild("eyes", ModelPartBuilder.create().uv(0, 0).cuboid(-3.5F, -4.0F, -6.0F, 3.0F, 2.0F, 3.0F, new Dilation(0.0F))
                .uv(0, 5).cuboid(0.5F, -4.0F, -6.0F, 3.0F, 2.0F, 3.0F, new Dilation(0.0F)), ModelTransform.pivot(0.0F, 0.0F, 0.0F));
    ...
    public static Animation getAttackAnimation(FrogTypes type) {
        return switch (type) {
            case BOSS_SOLDIER -> ModAnimations.BOSS_SOLDIER_FROG_ATTACK;
            case ARMED -> ModAnimations.ARMED_FROG_ATTACK;
            case EXPLOSIVE -> ModAnimations.EXPLOSIVE_FROG_ATTACK;
            case GRAPPLING -> ModAnimations.GRAPPLING_FROG_ATTACK;
            default -> ModAnimations.FROG_ATTACK;
        };
    }
}
