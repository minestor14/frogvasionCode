...
public class GrapplingFrogGoal extends Goal {
    ...
    public void tick() {
        ...
        if (target != null) {
            ...
            if (inLineOfSight && Math.abs(entity.getY()-entity.getTarget().getY()) <= 2.5) {
                this.attackTime++;
                ...
                if(this.attackTime >=11 && this.attackTime <=18) {
                    for(Entity e : this.entity.getWorld().getOtherEntities(this.entity, new Box(new Vec3d(x+0.5,y+0.5,z+0.5), new Vec3d(x-0.5,y-0.5,z-0.5)),(entity) -> (entity instanceof LivingEntity) && ((entity instanceof PlayerEntity)? (!((PlayerEntity) entity).isCreative() || !entity.isSpectator()):true))){
