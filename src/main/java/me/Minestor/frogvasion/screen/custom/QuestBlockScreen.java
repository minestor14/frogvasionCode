...
public class QuestBlockScreen extends HandledScreen<QuestBlockScreenHandler> {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        textRenderer.draw(objective, cx - (float) textRenderer.getWidth(objective) / 2, cy - (float) backgroundHeight /3 +5, 3618615,false,
                context.getMatrices().peek().getPositionMatrix(), context.getVertexConsumers(), TextRenderer.TextLayerType.NORMAL, 0, 15);
        textRenderer.draw(getToUse(quest.getType()), cx - (float) textRenderer.getWidth(getToUse(quest.getType())) / 2, cy - (float) backgroundHeight /3 + 20, 3618615, false,
                    context.getMatrices().peek().getPositionMatrix(), context.getVertexConsumers(), TextRenderer.TextLayerType.NORMAL, 0, 15);
    }
    private Text getToUse(QuestType type) {
        return switch (type) {
            case Collect -> objectiveItem;
            case Kill -> objectiveEntity;
            case Mine -> objectiveBlock;
            case Enchant -> objectiveEnchant;
    ...
    public class DoneButtonWidget extends IconButtonWidget {
        ...
        public void onPress() {
            if(!this.disabled){
                ...
                ClientPlayNetworking.send(ModMessages.UPDATE_QUEST_C2S, ModPackets.questUpdate(quest));
